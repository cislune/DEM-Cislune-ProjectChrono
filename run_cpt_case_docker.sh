...
OUTPUT_ROOT=${GRASP_CPT_OUTPUT_ROOT:-"$HOME/grasp-cpt-runs"}
...
if [[ $# -lt 1 ]]; then
    echo "usage: $0 MANIFEST [cpt_case_runner.py options]" >&2
    exit 64
fi
manifest=$1
shift
...
[[ -f "$ROOT/$manifest_rel" ]] || { echo "manifest not found: $ROOT/$manifest_rel" >&2; exit 66; }

mkdir -p "$OUTPUT_ROOT/_logs" "$CACHE_ROOT"
...
previous=
for argument in "$@"; do
    if [[ $previous == --stage ]]; then stage=$argument; break; fi
    previous=$argument
done
case_id=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["case_id"])' "$ROOT/$manifest_rel")
log_path="$OUTPUT_ROOT/_logs/$(date -u +%Y%m%dT%H%M%SZ)_${case_id}_${stage}.log"
...
docker run --rm --gpus all \
    ...
    -w /workspace \
    "$IMAGE" "$PYTHON_IN_CONTAINER" -u /workspace/cpt_case_runner.py "/workspace/$manifest_rel" \
    --output-root /outputs "$@" 2>&1 | tee "$log_path"
status=${PIPESTATUS[0]}
set -e

printf '\ncontainer_exit_status=%s\n' "$status" >> "$log_path"
docker run --rm -v "$OUTPUT_ROOT:/outputs" "$IMAGE" chown -R "$(id -u):$(id -g)" /outputs
docker run --rm -v "$CACHE_ROOT:/cache" "$IMAGE" chown -R "$(id -u):$(id -g)" /cache
exit "$status"
